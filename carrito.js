const platillosEnCarritoLasMarias = JSON.parse(localStorage.getItem("platillos-las-marias"));
console.log(platillosEnCarritoLasMarias);
...
function cargarPlatillosCarrito(){
	...
	if (platillosEnCarritoLasMarias && platillosEnCarritoLasMarias.length > 0 ) {
		...
		platillosEnCarritoLasMarias.forEach(platillo => {
...
function eliminarDelCarrito(e){
	...
	platillosEnCarritoLasMarias.splice(index, 1);
	cargarPlatillosCarrito();
...
function vaciarCarrito(){
	...
	Swal.fire({
  		...
  		if (result.isConfirmed) {
  			platillosEnCarritoLasMarias.length = 0;
			localStorage.setItem("platillos-las-marias", JSON.stringify(platillosEnCarritoLasMarias) );
			cargarPlatillosCarrito();
    		Swal.fire({
      		title: "Borrado",
...
function actualizarTotal(){
	const totalCalculado = platillosEnCarritoLasMarias.reduce((acc, platillo)=> acc +(platillo.precio * platillo.cantidad), 0);
